...
- **当日最高价 / 最低价**：来自指定交易日的不复权日K，腾讯主源与东方财富兜底均支持。
- **5日平均换手率**：保留5日累计换手率，新增含报告日在内最近5条交易日记录的算术平均；不足5条按实际记录数计算并注明。窗口内有缺失时显示“—”，推算值标 *。
- **龙虎榜机构资金**：展示机构买入总额、卖出总额、净买入和净卖出，单位万元。净买入为买入减卖出的正值，净卖出为卖出减买入的正值，反方向显示0。采用[东方财富机构买卖每日统计](https://data.eastmoney.com/stock/jgmmtj.html)的机构汇总数据，避免匿名机构席位在买卖榜重复相加；单日与多日累计榜按上榜原因分别展示。未披露与接口失败均标注，不当作零。

修改只影响新生成的报告；历史日期需在手动工作流重新运行以更新 HTML 和云盘 PDF。
新增计算及完整报表回归测试（Node.js 24）：`node --test tests/report-metrics.test.ts tests/report-render.test.ts`。
